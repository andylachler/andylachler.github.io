...
// ProjectVisual — the image layer of the CMS-lite system.
// Reads window.IMAGE_MANIFEST (generated by tools/build-image-manifest.js from
// the images/<project-id>/ folders). If an image exists for this project+kind,
// it renders over the TilePlaceholder pattern with a fade-in; the pattern
// remains the loading state and the fallback when no image (or a broken path)
// exists. Delete an image from the folder + rebuild the manifest and the tile
// cleanly reverts to its pattern. No component edits needed per image.
//
// Props:
//   projectId:  id used as the folder name under images/
//   kind:       'tile' | 'hero' — hero falls back to tile if absent
//   src:        explicit image URL — overrides manifest lookup (galleries)
      ...
      {showImg && (
        <img
          ...
          style={{
            ...
            objectFit: 'cover',
            opacity: loadedSrc === url ? 1 : 0,
            transform: hovered ? 'scale(1.03)' : 'scale(1)',
            transition: 'opacity 400ms ease-out, transform 600ms cubic-bezier(0.22,1,0.36,1)',
          }}
        />
      )}
    </>
...
Object.assign(window, { ArrowLink, TilePlaceholder, ProjectVisual, ProjectTile, tileBoxShadow, tileInset });
